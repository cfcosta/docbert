...
2. Split the document body into chunks if it exceeds the model's maximum document length (read from `config_sentence_transformers.json` locally, or fetched via hf-hub for remote model IDs; defaults to the built-in chunk size of ~4096 tokens if unavailable). Chunking strategy: character-based windowing with word-boundary adjustments, no overlap (to minimize chunk count)
