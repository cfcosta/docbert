...
1) Docbert now defaults to 519-token chunking when no model config is available.
   This matches ColBERT-Zero's training length, but not the 300-token ColBERT
   metadata defaults discussed above.
