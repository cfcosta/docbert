...
pub async fn get(
    ...
    let stored = state.config_db.get_persisted_llm_settings()?;
...
pub async fn update(
    ...
    state.config_db.set_persisted_llm_settings(&stored)?;
    Ok(Json(body))
}
...
mod tests {
    ...
    #[test]
    fn settings_get_prefers_stored_api_key_over_env() {
        let _guard = env_lock();
    ...
    #[test]
    fn settings_get_uses_env_api_key_when_stored_key_missing() {
        let _guard = env_lock();
        let (_tmp, state) = test_state();
        state
            .config_db
    ...
    #[tokio::test]
    async fn settings_update_clears_empty_api_key_and_absent_provider_model() {
        let (_tmp, state) = test_state();
        state
            .config_db
            .set_persisted_llm_settings(&PersistedLlmSettings {
    ...
    #[tokio::test]
    async fn settings_update_preserves_http_shape() {
        let (_tmp, state) = test_state();
